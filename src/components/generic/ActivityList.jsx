...
function ActivityList({ items, storageId, colClass, maxItems, hideProgressBar, cascadeMargin }) {
    ...
    const _shouldDisplayAsProgressItem = (item) => {
        // Only show progress bar if value is a number
        return item.progress !== null && item.progress !== undefined && !hideProgressBar && typeof item.value !== 'string';
    }
                        ...
                        {!_shouldDisplayAsProgressItem(item) && (
                            <ActivityListItem item={item}
                                ...
                                {typeof item.value === 'string' && isNaN(Number(item.value)) && (
                                    <span className={`text-highlight text-1 me-1 fw-bold`}>{item.value}</span>
                                )}
                            </ActivityListItem>
                        )}
                    </Col>
        ...
        <div className={`activity-list-item`} style={customStyle}>
            ...
            <div className={`content-wrapper w-100`}>
                <div className={`d-flex justify-content-between font-family-subheadings`}>
                    ...
                    {item.progress !== null && item.progress !== false && typeof item.value !== 'string' && (
